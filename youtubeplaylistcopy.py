...
if YOUTUBE_API_KEY:
    ...
    st.error("YouTube Data API key not found. Please set the YOUTUBE_API_KEY environment variable.")
    youtube = None
    ...
    try:
        while True:
            ...
            for item in response['items']:
                ...
                video_urls.append({'video_id': video_id, 'title': video_title, 'url': f"youtu.be/{video_id}"})
...
def summarize_text_with_gemini(text):
    ...
    if text in ["No transcript found.", "Transcripts disabled for this video.", "Error fetching transcript:"]:
...
if st.button("Get Summaries"):
    if not GEMINI_API_KEY or not YOUTUBE_API_KEY:
        st.warning("Please set both `GEMINI_API_KEY` and `YOUTUBE_API_KEY` environment variables to proceed.")
        ...
        if not playlist_id:
            ...
            st.info("Fetching video details and transcripts. This might take a while for large playlists...")

            with st.spinner("Fetching videos from playlist..."):
                video_details = get_playlist_video_urls(playlist_id)

            if not video_details:
                st.warning(f"No videos found in playlist {playlist_id} or an error occurred during fetching.")
            else:
                results_df = pd.DataFrame(columns=['Video Title', 'Video URL', 'Summary'])
                ...
                for i, detail in enumerate(video_details):
                    ...
                    status_text.text(f"Processing video {i+1}/{len(video_details)}: {video_title}")
                    ...
                    # Summarize
                    summary = ""
                    if transcript and not transcript.startswith(("No transcript found", "Transcripts disabled", "Error fetching")):
                        with st.spinner(f"Summarizing '{video_title}'..."):
                            summary = summarize_text_with_gemini(transcript)
                    else:
                        summary = transcript # Set summary to the error message or "No transcript found"
